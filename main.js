...
async function insertServerInfo(id, ip, osType, port, encryptedUser, encryptedPassword, serverName) {
  let connection;
...
ipcMain.handle("add-server", async (event, serverData) => {
  ...
  try {
    ...
    await insertServerInfo(id, ip, os, port, encryptedUser, encryptedPassword, serverName);
    return { success: true };
  } catch (error) {
...
async function saveLogToDatabase(
  ...
  try {
    ...
    if (Array.isArray(dumpFileInfo)) {
      dumpFileInfo.forEach(file => {
        if (file && file.filePath && file.filePath.toLowerCase().endsWith('.dmp')) {
    ...
    } else if (dumpFileInfo && dumpFileInfo.filePath && dumpFileInfo.filePath.toLowerCase().endsWith('.dmp')) {
