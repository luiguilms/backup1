document.addEventListener("DOMContentLoaded", () => {
  ...
  form.addEventListener('submit', async (event) => {
    ...
    window.localStorage.setItem('connectionData', JSON.stringify({ os, ip, port, username, password }));
    ...
    try {
      ...
      if (isConnected) {
        ...
        try {
          const { logDetails, dumpFileInfo } = await window.electron.getLogDetails(directoryPath, ip, port, username, password, os);

          if (logDetails) {
            logDateSpan.textContent = logDetails.dateTime;
            durationSpan.textContent = logDetails.duration;
          ...
          dumpPathSpan.textContent = dumpFileInfo ? dumpFileInfo.filePath : 'N/A';
          dumpSizeSpan.textContent = dumpFileInfo ? `${dumpFileInfo.fileSize} MB` : 'N/A';
        ...
        resultDiv.style.display = 'block';
        resultDiv.textContent = `Could not connect to ${ip}:${port}`;
      }
    } catch (error) {
      resultDiv.style.display = 'block';
